...
class BaseDataset(Dataset):
    ...
    def load_data(self):
        ...
        for cnt, data_path in enumerate(self.data_path):
            ...
            if self.config['use_cache'] or is_ddp():
                ...
                reuse_cache = os.path.exists(self.cache_path) and os.path.exists(file_list_path) and self.config.get('overwrite_cache', False) is False
                if reuse_cache:
                    print('Warning: cache path {} already exists, skip '.format(self.cache_path))
                    file_list = self.get_data_list(data_usage_this_dataset)
                    if len(file_list) == 0:
                        print('Warning: cached file list is empty, rebuilding cache')
                        reuse_cache = False
                if not reuse_cache:
    ...
    def process(self, internal_format):
        ...
        has_map_features = any(
            len(info['map_infos'].get(feature_key, [])) > 0
            for feature_key in ['lane', 'road_line', 'road_edge', 'stop_sign', 'crosswalk', 'speed_bump']
        )
        if has_map_features:
        ...
        if self.config['remove_outliers'] and map_polylines_mask.any(): #remove agents that are further away than 5m from the nearest lane at the central time step -> filtering step done in fmae/emp data preprocessing
